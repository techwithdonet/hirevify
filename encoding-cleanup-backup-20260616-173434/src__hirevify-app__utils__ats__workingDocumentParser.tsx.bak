  ...
  private extractProfessionalSummary(text: string): string {
    ...
    for (let i = 1; i < Math.min(15, lines.length); i++) {
      ...
      if (line.includes('.') && line.length > 50 && line.length < 500) {
        ...
        const isNotJobTitle = !lowerLine.includes(' at ') && 
                             !lowerLine.includes(' - ') && 
    ...
    for (let i = 1; i < Math.min(20, lines.length - 2); i++) {
      ...
      if (currentLine.length > 30 && nextLine.length > 30 &&
          !currentLine.includes('@') && !nextLine.includes('@') &&
          !this.isHeaderLine(currentLine) && !this.isHeaderLine(nextLine)) {
