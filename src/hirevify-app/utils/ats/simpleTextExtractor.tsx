 ...
 */
 ...
 console.log('Done Successfully extracted text via UTF-8 reading');
 ...
 console.log(`Done Successfully extracted text via ${encoding} encoding`);
 return encodedResult;
 }
 } catch (error) {
 console.log(`Error Failed to read with ${encoding} encoding:`, error.message);
 }
 }
 ...
 console.log('Done Successfully extracted text from simple PDF');
 return pdfText;
 }
 }
 ...
 console.error('Error SimpleTextExtractor failed:', error);
 throw error;
 }
 }
 ...
 */
 ...
 const cleanText = match.replace(/[()[\]]/g, '').replace(/BT|ET/g, '').replace(/Tf|Td|TD|Tm|TL|Tc|Tw|Tz|TJ|Tj|'/g, '').replace(/\s+/g, ' ').trim();
 ...
 */
 ...
 return text
 // Remove excessive whitespace.replace(/\s+/g, ' ')
 // Remove common PDF artifacts.replace(/[^\x20-\x7E\n\r\t]/g, '')
 // Remove numbers that are likely page numbers or formatting.replace(/^\d+\s*$/gm, '')
 // Clean up line breaks.replace(/\n\s*\n\s*\n/g, '\n\n').trim();
 }
