...
class SmartDocumentProcessor {
 ...
 const textMatches = [...textObj.matchAll(/\((.*?)\)[\s]*Tj/g),...textObj.matchAll(/\[(.*?)\][\s]*TJ/g),...textObj.matchAll(/\((.*?)\)[\s]*'/g)
 ];
 ...
 extractedText = extractedText.replace(/\\n/g, '\n').replace(/\\r/g, '\r').replace(/\\t/g, '\t').replace(/\\/g, '').replace(/\s+/g, ' ').replace(/[^\x20-\x7E\n\r\t]/g, ' ') // Remove non-ASCII.trim();
...
If this is a text-rich document and you're seeing this message, please try converting to a plain text file.`;
 ...
 text = text.replace(/\x00+/g, ' ') // Remove null characters.replace(/[^\x20-\x7E\n\r\t]/g, ' ') // Remove non-printable characters.replace(/\s+/g, ' ') // Normalize whitespace.replace(/(.)\1{10,}/g, '$1') // Remove long repeated characters.trim();
 ...
 /[a-zA-Z]/.test(cleanLine) &&!(/^[^a-zA-Z]*$/.test(cleanLine)); // Not just symbols/numbers
 });
 ...
 // Remove control characters except newlines and tabs.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '')
 // Normalize line breaks.replace(/\r\n/g, '\n').replace(/\r/g, '\n')
 // Remove multiple consecutive newlines.replace(/\n{3,}/g, '\n\n')
 // Trim.trim();
 }
