 ...
 */
 ...
 console.log('Done Validation completed, final confidence:', validatedData.extractionMetadata.confidence);

 return validatedData;

 } catch (error) {
 console.error('Error Enterprise parsing failed:', error);
 ...
 */
 ...
 console.log(`Done Enhanced PDF extraction complete with ${enhancedResult.extractedData.workExperience.length} work experiences and ${enhancedResult.extractedData.education.length} education entries`);
 ...
 console.log(`Done Basic PDF extraction complete: ${fullText.length} total characters`);
 ...
 */
 ...
 console.log(`Done DOCX extraction complete: ${result.value.length} characters`);
 ...
 */
 ...
 console.log(`Done OCR extraction complete: ${text.length} characters`);
 return text.trim();
 } else {
 ...
 */
 ...
 validated.experience = validated.experience.map((exp: any, index: number) => ({...exp,
 id: exp.id || `exp_${index + 1}`
 }));

 validated.education = validated.education.map((edu: any, index: number) => ({...edu,
 ...
 */
 ...
 words.length <= 4 &&!emailRegex.test(line) &&!phoneRegex.test(line) &&!/^(resume|cv|curriculum)/i.test(line);
 })?.trim() || 'Candidate Name';
 ...
 */
 ...
 const summaryLines = lines.slice(i + 1, i + 4).filter(line => line.length > 20 &&!this.isHeaderLine(line));
 ...
 */
 ...
 const candidateName = fileNameParts.filter(part => part.length > 1 &&!['resume', 'cv'].includes(part)).slice(0, 2).map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(' ') || 'Professional Candidate';
