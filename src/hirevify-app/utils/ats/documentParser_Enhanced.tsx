 ...
 */
 ...
 console.log(`Done Found name via pattern on line ${i + 1}: "${extractedName}" from: "${line}"`);
 ...
 if (extractedName && this.isValidName(extractedName)) {
 console.log(`Done Found name after section header on line ${i + 2}: "${extractedName}" after header: "${currentLine}"`);
 return extractedName;
 }
 }
 }
 ...
 const extractedName = this.cleanExtractedName(match[1]);
 if (extractedName && this.isValidName(extractedName)) {
 console.log(`Done Found name via key-value pattern: "${extractedName}"`);
 return extractedName;
 }
 }
 }
 ...
 const extractedName = this.cleanExtractedName(match[1]);
 if (extractedName && this.isValidName(extractedName)) {
 console.log(`Done Found name via table pattern: "${extractedName}"`);
 return extractedName;
 }
 }
 }
 ...
 const firstValidLines = lines.slice(0, 15).filter(line => line.length >= 3 && line.length <= 80).filter(line =>!/[@\.com|www\.|http|phone|tel|email|address]/i.test(line)).filter(line =>!/^\d+/.test(line)) // Exclude lines starting with numbers.filter(line =>!/(resume|curriculum|vitae|cv)$/i.test(line)).filter(line =>!/^(objective|summary|profile|experience|education|skills)/i.test(line));
 
 for (const line of firstValidLines) {
 const extractedName = this.cleanExtractedName(line);
 if (extractedName && this.isValidName(extractedName)) {
 console.log(`Done Found name via enhanced first-line analysis: "${extractedName}"`);
 return extractedName;
 }
 }
 ...
 const extractedName = this.cleanExtractedName(match[1]);
 if (extractedName && this.isValidName(extractedName)) {
 console.log(`Done Found name via context pattern: "${extractedName}"`);
 return extractedName;
 }
 }
 }
 ...
 const extractedName = this.cleanExtractedName(match[1]);
 if (extractedName && this.isValidName(extractedName)) {
 console.log(`Done Found name via emphasis pattern: "${extractedName}"`);
 return extractedName;
 }
 }
 }
 
 console.log('Warning No name found using any of the 7 extraction strategies');
 return undefined;
 }
 ...
 */
 ...
 return nameText.trim().replace(/[^\w\s\-'.]/g, '') // Remove special chars except common name chars.replace(/\s+/g, ' ') // Normalize whitespace.replace(/^(mr|mrs|ms|dr|prof|professor)\.?\s*/i, '') // Remove titles.replace(/\b(resume|cv)\b/i, '') // Remove document type words.trim();
 }
