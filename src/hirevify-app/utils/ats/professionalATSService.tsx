 ...
 */
 ...
 console.log(`Done Parsing completed in ${parsingTime}ms with confidence: ${parsedData.extractionMetadata.confidence}`);
 ...
 console.log(`Done Scoring completed in ${scoringTime}ms. Overall score: ${atsScore.overall}`);
 ...
 console.error('Error Professional ATS processing failed:', error);
 ...
 */
 ...
 const cleanName = filename.toLowerCase().replace(/\.(pdf|doc|docx)$/, '').replace(/[-_]/g, ' ').split(' ').filter(part => 
 part.length > 1 &&!['resume', 'cv', 'curriculum', 'vitae', 'final', 'updated'].includes(part)
 ).slice(0, 2).map(part => part.charAt(0).toUpperCase() + part.slice(1)).join(' ');
