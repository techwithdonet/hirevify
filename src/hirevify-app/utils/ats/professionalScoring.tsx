 ...
 */
 ...
 const words = jobDescription.toLowerCase().split(/\s+/).filter(word => word.length > 3).filter(word =>!/^(the|and|for|with|that|this|have|will|are|from)$/.test(word));
 ...
 return Object.entries(wordFreq).sort(([,a], [,b]) => b - a).slice(0, 20).map(([word]) => word);
 }
 ...
 */
 ...
 return industryKeywords.filter(keyword =>!currentKeywords.includes(keyword.toLowerCase())).slice(0, 5);
 }
