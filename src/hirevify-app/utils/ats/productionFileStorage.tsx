 ...
 */
 ...
 const { error } = await this.supabase.storage.from(this.bucketName).list('', { limit: 1 });
 ...
 console.log('Done Storage bucket is accessible');
 }
 } catch (error) {
 ...
 */
 ...
 if (result.success) {
 console.log('Done Server-side upload successful:', result.filePath);
 return {
 success: true,
 ...
 // console.log('Done File uploaded successfully:', data.path);
 ...
 */
 ...
 try {
 console.log('ѕ Deleting file:', filePath);
 ...
 console.log('Done File deleted successfully');
 return true;
 ...
 */
 ...
 try {
 const { data, error } = await this.supabase.storage.from(this.bucketName).list(`resumes/${userId}/`, {
 limit: 100,
 offset: 0
 });
