 ...
 */
 ...
 const overall = this.calculateWeightedOverallScore({...basicScores,...industryScores,...aiAnalysis
 });
 ...
 console.log('Done Enterprise scoring completed with overall score:', overall);
 return enterpriseScore;

 } catch (error) {
 console.error('Error Enterprise scoring failed:', error);
 ...
 */
 ...
 const allSkills = [...resumeData.skills.technical,...resumeData.skills.tools,...resumeData.skills.frameworks,...resumeData.skills.soft
 ];
 ...
 */
 ...
 const words = jobDescription.toLowerCase().split(/\s+/).filter(word => word.length > 2).filter(word =>!/^(the|and|for|with|that|this|have|will|are|from|you|your|our)$/.test(word));
 ...
 return Object.entries(wordFreq).sort(([,a], [,b]) => b - a).slice(0, 25).map(([word]) => word);
 }
