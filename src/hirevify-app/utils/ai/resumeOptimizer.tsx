 ...
 */
 ...
 const allResumeSkills = [...resume.skills.technical,...resume.skills.soft,...resume.skills.languages
 ].map(skill => skill.toLowerCase());

 const requiredSkills = [...jobDescription.requirements,...jobDescription.preferredSkills
 ...
 const missingSkills = requiredSkills.filter(skill =>!allResumeSkills.some(resumeSkill => 
 resumeSkill.includes(skill.toLowerCase()) || 
 skill.toLowerCase().includes(resumeSkill)
 )
 );
 ...
 */
 ...
 if (personalInfo.Link || personalInfo.GitBranch || personalInfo.portfolio) score += 2;
 ...
 const nonStandardSections = resume.sections.filter(section =>!standardSections.includes(section.type)
 );
 ...
 recommendations.push('Add missing contact information (Link, portfolio)');
 ...
 const missingKeywords = jobKeywords.filter(keyword =>!resumeText.includes(keyword.toLowerCase())
 );
...
experiences.forEach((exp, index) => {
 const achievements = Array.isArray(exp?.achievements)? exp.achievements: typeof exp?.achievements === 'string'? exp.achievements.split('\n').map((item: string) => item.trim()).filter(Boolean): [];
 ...
 */
 ...
 const skillsText = [...(skills.technical || []),...(skills.soft || [])
 ].join(', ');
 ...
 */
 ...
 try {
 const currentSkillsList = [...(currentSkills.technical || []),...(currentSkills.soft || []),...(currentSkills.languages || [])
 ].join(', ');
...
Current Skills: ${[...(resumeData.skills?.technical || []),...(resumeData.skills?.soft || [])
].join(', ')}
