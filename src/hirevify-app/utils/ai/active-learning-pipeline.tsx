...
class ActiveLearningPipeline {
 ...
 diversityWeight: 0.3,
 performanceThreshold: 0.85,...config
 };
 ...
 */
 ...
 return instances.filter(instance => instance.uncertainty >= this.config.uncertaintyThreshold).sort((a, b) => b.uncertainty - a.uncertainty).slice(0, this.config.batchSize);
 }
 ...
 */
 ...
 const scoredInstances = instances.map(instance => ({...instance,
 combinedScore: (instance.uncertainty * uncertaintyWeight) + 
 (this.calculateMaxDiversityScore(instance, instances) * diversityWeight)
 }));

 return scoredInstances.sort((a, b) => b.combinedScore - a.combinedScore).slice(0, this.config.batchSize);
 }
 ...
 */
 ...
 const avgFieldUncertainty = fieldUncertainties.length > 0? fieldUncertainties.reduce((sum, u) => sum + u, 0) / fieldUncertainties.length: uncertainty;
 ...
 */
 ...
 const recentInstances = Array.from(this.learningInstances.values()).filter(inst => (Date.now() - inst.timestamp.getTime()) < 60 * 60 * 1000); // Last hour
 ...
 */
 ...
 const trainingData = Array.from(this.learningInstances.values()).filter(instance => instance.feedbackType === 'correction').map(instance => ({
 ...
 */
 ...
 const recentInstances = Array.from(this.learningInstances.values()).filter(inst => (Date.now() - inst.timestamp.getTime()) < 7 * 24 * 60 * 60 * 1000); // Last week
 ...
 */
 ...
 const recentInstances = Array.from(this.learningInstances.values()).filter(inst => (Date.now() - inst.timestamp.getTime()) < 24 * 60 * 60 * 1000).slice(-10);
 ...
 */
 ...
 console.log(`Done Imported ${data.instances.length} learning instances`);
 }
}
