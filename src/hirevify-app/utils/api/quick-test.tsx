...
export async function runQuickConnectivityTest(): Promise<{
 ...
 console.log(`${isWorking? 'Done': 'Error'} ${endpoint.name}: ${response.status}`);
 ...
 let summary;
 if (allWorking) {
 summary = `Done All ${testEndpoints.length} endpoints are working correctly!`;
 } else if (anyWorking) {
 summary = `Warning ${workingCount}/${testEndpoints.length} endpoints are working. Some features may be limited.`;
 } else {
 summary = `Error No endpoints are accessible. Backend appears to be completely down.`;
 }
