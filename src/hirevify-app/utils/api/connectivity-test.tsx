...
export async function runConnectivityTest(accessToken?: string): Promise<ConnectivityTestResult> {
 ...
 try {
 console.log('1ï Testing main server health...');
 ...
 if (response.ok) {
 console.log('Done No-auth endpoint works - proceeding to main health check');
 } else {
 console.log(`Warning No-auth endpoint returned ${response.status} - trying main health check anyway`);
 }
 } catch (noAuthError) {
 console.log('Warning No-auth endpoint failed - trying main health check anyway:', noAuthError.message);
 }
 ...
 console.log('Done Main server health check passed:', data);
 } else {
 const errorText = await response.text();
 result.tests.mainServer.error = `HTTP ${response.status}: ${errorText}`;
 console.log('Error Main server health check failed:', response.status, errorText);
 }
 } catch (error) {
 result.tests.mainServer.error = error.message;
 console.log('Error Main server connectivity error:', error.message);
 ...
 try {
 console.log('2ï Testing integration service health...');
 const startTime = Date.now();
 ...
 console.log(`Done ${endpoint.name} succeeded`);
 break;
 } else {
 console.log(`Warning ${endpoint.name} failed with status ${response.status}`);
 }
 } catch (endpointError) {
 console.log(`Warning ${endpoint.name} failed with error:`, endpointError.message);
 }
 }
 ...
 console.log('Done Integration service health check passed:', data);
 } else {
 const errorText = await response.text();
 result.tests.integrationService.error = `HTTP ${response.status}: ${errorText}`;
 console.log('Error Integration service health check failed:', response.status, errorText);
 }
 } catch (error) {
 result.tests.integrationService.error = error.message;
 console.log('Error Integration service connectivity error:', error.message);
 ...
 try {
 console.log('3ï Testing authentication with integration service...');
 ...
 console.log('Done Authentication test passed:', data);
 } else {
 const errorData = await response.json().catch(() => ({}));
 result.tests.authentication.error = `HTTP ${response.status}: ${errorData.error || 'Authentication failed'}`;
 console.log('Error Authentication test failed:', response.status, errorData);
 ...
 console.log('Error Authentication test error:', error.message);
...
export function formatConnectivityTestResults(result: ConnectivityTestResult): string {
 ...
 report += ` Has Public Key: ${result.environment.hasPublicKey? 'Done': 'Error'}\n`;
 report += ` Has User Token: ${result.environment.hasUserToken? 'Done': 'Error'}\n\n`;
 
 // Test Results
 report += ' Test Results:\n';
 
 const serverTest = result.tests.mainServer;
 report += ` 1. Main Server: ${serverTest.success? 'Done': 'Error'}\n`;
 ...
 report += ` 2. Integration Service: ${integrationTest.success? 'Done': 'Error'}\n`;
 ...
 report += ` 3. Authentication: ${authTest.success? 'Done': 'Error'}\n`;
