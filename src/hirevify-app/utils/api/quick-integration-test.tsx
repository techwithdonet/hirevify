...
export async function quickIntegrationTest() {
 ...
 console.log(`Done ${endpoint}: SUCCESS (${response.status})`);
 console.log(' Data:', result.data);
 } catch {
 result.text = await response.text();
 console.log(`Done ${endpoint}: SUCCESS (${response.status}) - Text response`);
 ...
 results.push(result);
 console.log(`Error ${endpoint}: NETWORK ERROR`);
 ...
 console.log('\n SUMMARY:');
 console.log(`Done Working: ${working.length}/${results.length}`);
 console.log(`Error Failed: ${failed.length}/${results.length}`);
 
 if (working.length > 0) {
 console.log('\nDone Working endpoints:');
 working.forEach(r => console.log(` - ${r.endpoint} (${r.status})`));
 }
 
 if (failed.length > 0) {
 console.log('\nError Failed endpoints:');
