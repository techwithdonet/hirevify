 ...
 */
 ...
 success: response.ok,
 message: response.ok? `${name} is healthy (${responseTime}ms)`: `${name} returned ${response.status}`,
 details: {
 endpoint: name,
 ...
 console.error(`Error ${name} failed:`, error.message);
 ...
 */
 ...
 if (result.success) {
 console.log(`Done Backend connectivity confirmed via ${endpoint.name}`);
 return {
 success: true,
 ...
 console.log(`Warning ${endpoint.name} failed, trying next...`);
 }

 } catch (error) {
 const errorMessage = error.message || 'Unknown error';
 console.log(`Warning ${endpoint.name} error: ${errorMessage}`);
