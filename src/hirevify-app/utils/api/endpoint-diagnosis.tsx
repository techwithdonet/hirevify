...
export async function diagnoseAllEndpoints(): Promise<DiagnosisReport> {
 ...
 if (response.ok) {
 successfulCount++;
 console.log(`Done ${endpoint.name}: ${response.status} (${responseTime}ms)`);
 } else {
 failedCount++;
 console.log(`Error ${endpoint.name}: ${response.status} (${responseTime}ms)`);
 }
 ...
 recommendations.push('Done Main server is working correctly.');
 }
 ...
 recommendations.push('Done Integration backup endpoints are working.');
 }
 ...
 recommendations.push('Done Integration service is working correctly.');
 }
