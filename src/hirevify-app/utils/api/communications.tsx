...
function normalizeUserType(profile: ProfileRow): "recruiter" | "candidate" {
 return profile.role === "recruiter" || profile.user_type === "recruiter"? "recruiter": "candidate";
}
...
async function getProfileMap(userIds: string[]) {
 ...
 const { data, error } = await supabase.from("profiles").select("id, full_name, name, email, role, user_type, company_name, title, avatar_url").in("id", uniqueIds);
...
export class CommunicationsAPI {
 ...
 const { data, error } = await supabase.from("messages").select("*").or(`sender_id.eq.${currentUserId},recipient_id.eq.${currentUserId}`).order("created_at", { ascending: false });
 ...
 if (
 conversation &&
 message.recipientId === currentUserId &&!message.read
 ) {
