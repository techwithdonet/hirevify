...
export async function quickBackendCheck(): Promise<boolean> {
 ...
 console.log(`Done Quick test successful: ${url} (${response.status})`);
 return true;
 }
 
 console.log(`Warning Quick test non-ok response: ${url} (${response.status})`);
 } catch (error) {
 console.log(`Error Quick test failed: ${url} - ${error.message}`);
 }
 }
 
 console.log('Error All quick tests failed');
 return false;
}
...
async function testSupabaseBasics(): Promise<boolean> {
 ...
 if (response.ok) {
 console.log('Done Supabase basic connectivity successful');
 return true;
 } else {
 console.log(`Warning Supabase returned ${response.status}`);
 return false;
 }
 
 } catch (error) {
 console.log('Error Supabase basic connectivity failed:', error.message);
 return false;
 }
}
...
async function testBackendServer(): Promise<boolean> {
 ...
 console.log(`Done Backend endpoint successful: ${endpoint} (${response.status})`);
 return true;
 } else {
 console.log(`Warning Backend endpoint ${endpoint} returned ${response.status}`);
 }
 
 } catch (error) {
 console.log(`Error Backend endpoint ${endpoint} failed:`, error.message);
 }
 }
 
 console.log('Error All backend endpoints failed');
 return false;
}
...
async function testIntegrationEndpoints(): Promise<boolean> {
 ...
 if (response.ok || response.status === 401) {
 console.log(`Done Integration endpoint successful: ${endpoint} (${response.status})`);
 return true;
 } else {
 console.log(`Warning Integration endpoint ${endpoint} returned ${response.status}`);
 ...
 console.log(`Error Integration endpoint ${endpoint} failed:`, error.message);
 }
 }
 
 console.log('Error All integration endpoints failed');
 return false;
}
...
export async function testAuthenticatedIntegrationAccess(accessToken: string): Promise<ConnectivityTestResult> {
 ...
 console.log('Done Authenticated integration access successful');
 ...
 console.log(`Error Authenticated integration access failed: ${response.status}`);
