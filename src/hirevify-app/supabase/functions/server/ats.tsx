...
RESPONSE FORMAT (PURE JSON, NO EXPLANATIONS):
{
  "personalInfo": {
    ...
    "Link": "EXACT Link URL if found or empty string",
    "GitBranch": "EXACT GitBranch URL if found or empty string",
...
function ultraSafeTextExtraction(text: string) {
  ...
  const comprehensiveSkills = [
    ...
    'AWS', 'Azure', 'GCP', 'Docker', 'Kubernetes', 'Terraform', 'CloudFormation', 'Ansible', 'Jenkins', 'GitLab CI', 'GitBranch Actions', 'CircleCI',
  ...
  return {
    personalInfo: {
      ...
      phone: phoneMatch || '',
      location: location || '',
      Link: text.match(/(?:Link\.com\/in\/|Link\.com\/pub\/)[\w-]+/i)?.[0] || '',
      GitBranch: text.match(/GitBranch\.com\/[\w-]+/i)?.[0] || '',
...
function basicExtraction(text: string) {
  ...
  return {
    personalInfo: {
      ...
      location: '',
      Link: text.match(/Link\.com\/in\/[\w-]+/i)?.[0] || '',
      GitBranch: text.match(/GitBranch\.com\/[\w-]+/i)?.[0] || '',
      portfolio: ''
    },
    professionalSummary: '',
