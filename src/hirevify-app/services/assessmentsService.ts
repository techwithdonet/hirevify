...
class AssessmentsService {
 ...
 const { data, error } = await supabase.from('assessment_results').select('*').eq('user_id', String(userId)).order('completed_at', { ascending: false });
 ...
 const { data, error } = await supabase.from('assessment_results').upsert(payload, { onConflict: 'user_id,assessment_id' }).select('*').single();
