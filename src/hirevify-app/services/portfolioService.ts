...
class PortfolioService {
 ...
 const { data, error } = await supabase.from('candidate_portfolio_items').select('*').eq('user_id', user.id).order('created_at', { ascending: false });
 ...
 const { data, error } = await supabase.from('candidate_portfolio_items').insert({
 user_id: user.id,
 title: input.title,
 ...
 const { data, error } = await supabase.from('candidate_portfolio_items').update(updateData).eq('id', id).eq('user_id', user.id).select().single();
 ...
 const { error } = await supabase.from('candidate_portfolio_items').delete().eq('id', id).eq('user_id', user.id);
