 ...
 */
 ...
 const { data, error } = await this.supabase.from('saved_jobs').select(
 `
 *,
 ...
 `
 ).eq('candidate_id', candidateId).order('saved_at', { ascending: false }).returns<any[]>();
 ...
 */
 ...
 const { data, error } = await this.supabase.from('saved_jobs').insert([{ candidate_id: candidateId, job_id: jobId }]).select().single<SavedJob>();
 ...
 */
 ...
 const { error } = await this.supabase.from('saved_jobs').delete().eq('candidate_id', candidateId).eq('job_id', jobId);
 ...
 */
 ...
 const { data, error } = await this.supabase.from('saved_jobs').select('id').eq('candidate_id', candidateId).eq('job_id', jobId).single();
 ...
 */
 ...
 const { count, error } = await this.supabase.from('saved_jobs').select('*', { count: 'exact', head: true }).eq('candidate_id', candidateId);
