...
class JobsService {
  ...
  async getPublishedJobs(filters?: JobFilter & { limit?: number; offset?: number }) {
    let query = this.supabase
      .from('jobs')
      .select(`*, recruiter_profile:recruiter_id(company_name, company_logo_url)`)
      .eq('status', 'published')
  ...
  async getJob(jobId: string) {
    const { data, error } = await this.supabase
      .from('jobs')
      .select(`*, recruiter_profile:recruiter_id(company_name, company_logo_url, verified_recruiter)`)
      .eq('id', jobId)
      .single<JobWithRecruiter>();
  ...
  async getRecruiterStats(recruiterId: string) {
    const { data: jobs, error: jobsError } = await this.supabase
      ...
      .eq('recruiter_id', recruiterId)
      .returns<{ status: string; applications_count: number }[]>();
