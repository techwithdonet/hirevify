...
export function RecruiterDashboard({ 
 ...
 const conversations = await CommunicationsAPI.getConversations();
 ...
 <span className="text-xs text-slate-400">{formatMessageTime(conversation.lastMessage.createdAt)}</span>
 </div>
 <p className="truncate text-xs text-slate-500">{conversation.lastMessage.message}</p>
 </div>
