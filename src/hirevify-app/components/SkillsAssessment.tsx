...
export function SkillsAssessment({ onBack, userType, onCreateCustomAssessment }: SkillsAssessmentProps) {
 ...
 const { data, error } = await supabase.from('skills_assessments').select('*').eq('status', 'active').order('created_at', { ascending: false });
 ...
 const { data: loadedQuestions, error: questionsError } = await supabase.from('assessment_questions').select('*').in('assessment_id', assessmentIds).order('sort_order', { ascending: true });
...
const mappedAssessments: Assessment[] = (data || []).map((item: any) => ({
 ...
 questions: questionRows.filter((question: any) => question.assessment_id === item.id).map((question: any, index: number) => ({
 ...
 const { data: loadedQuestions, error } = await supabaseClient.from('assessment_questions').select('*').eq('assessment_id', assessment.id).order('sort_order', { ascending: true });
 ...
 const assessmentToStart = {...assessment,
 questions,
 };
 ...
 const optionIndex = Array.isArray(question.options)? question.options.findIndex((option: any) => normalizeAnswer(option) === selectedAnswer): -1;

 const correctOptionIndex = Array.isArray(question.options)? question.options.findIndex((option: any) => normalizeAnswer(option) === correctAnswer): -1;
