...
export function AISmartNotifications({ onBack, onUpgrade, onUpdateUnreadCount }: AISmartNotificationsProps) {
 ...
 {filterCategory!== 'all' || filterPriority!== 'all'? 'No notifications match your current filters': 'You\'re all caught up! AI will notify you when something important happens.'
 }
 </p>
 </Card>
 ): (
 ...
 <Card 
 key={notification.id} 
 className={`p-6 transition-all duration-200 ${!notification.read? 'ring-2 ring-primary/20 bg-primary/5': ''
 }`}
 >
 ...
 onCheckedChange={(checked) => 
 updatePreferences('ai_features', {...preferences.ai_features, 
 ...
 onCheckedChange={(checked) => 
 updatePreferences('delivery', {...preferences.delivery, 
 immediate: checked 
 })
 }
 />
 ...
 onCheckedChange={(checked) => 
 updatePreferences('delivery', {...preferences.delivery, 
 quiet_hours: {...preferences.delivery.quiet_hours, 
 enabled: checked 
 }
 })
 }
