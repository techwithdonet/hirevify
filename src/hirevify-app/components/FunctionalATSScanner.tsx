...
export function FunctionalATSScanner() {
  ...
  const extractTextFromFile = async (file: File): Promise<string> => {
    try {
      ...
      console.log('ðŸš€ Starting document parsing with WorkingDocumentParser...');
      const result = await parser.parseDocument(file);
      
      console.log('ðŸ“„ Parser result received:', result);
      ...
      if (result.extractionMetadata?.confidence && result.extractionMetadata.confidence > 0.5) {
        ...
        if (result.personalInfo) {
          ...
          if (result.personalInfo.Link) extractedText += `${result.personalInfo.Link}\n`;
          if (result.personalInfo.GitBranch) extractedText += `${result.personalInfo.GitBranch}\n`;
          extractedText += '\n';
        }
        ...
        console.log('âœ… Successfully converted structured data to text:', extractedText.substring(0, 300) + '...');
  ...
  const extractWorkExperience = (text: string, lines: string[]): WorkExperience[] => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (inExperienceSection) {
        ...
        // Look for dates
        const datePattern = /(\d{4})\s*[-â€“]\s*(\d{4}|present)/i;
  ...
  const extractSkills = (text: string, lines: string[]): Skills => {
    ...
    for (const line of lines) {
      ...
      if (inSkillsSection) {
        ...
        const skills = line.split(/[,â€¢Â·|]/).map(s => s.trim()).filter(s => s.length > 0);
  ...
  const calculatePersonalInfoConfidence = (info: PersonalInfo): number => {
    ...
    if (info.Link) score += 10;
    if (info.GitBranch) score += 5;
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {scanResult.extractedData.personalInfo.Link && (
                      <div className="space-y-2">
                    ...
                    {scanResult.extractedData.personalInfo.GitBranch && (
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-muted-foreground">GitBranch</label>
                        <p className="text-foreground flex items-center gap-2">
                          <ExternalLink className="w-4 h-4" />
                          {scanResult.extractedData.personalInfo.GitBranch}
                        </p>
