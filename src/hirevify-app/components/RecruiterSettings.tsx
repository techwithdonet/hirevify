...
export function RecruiterSettings({ onBack, onUpgrade }: RecruiterSettingsProps) {
 ...
 const { data: recruiterProfile } = await supabase.from('recruiter_profiles').select('*').eq('id', profile?.id || user.id).maybeSingle();
 ...
 const { data: verifyRow, error: verifyError } = await supabase.from('recruiter_profiles').select('id, company_name, contact_person, phone, website, industry, company_size, location, profile_completeness, profile_completed, updated_at').eq('id', recruiterProfileId).single();
