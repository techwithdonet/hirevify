...
const [currentStep, setCurrentStep] = useState(0);
 ...
 const { data: candidateProfile, error: candidateError } = await supabase.from('candidate_profiles').select('*').eq('user_id', authData.user.id).maybeSingle();
 ...
 experience: candidateProfile?.experience_summary && experienceLevels.includes(candidateProfile.experience_summary)? candidateProfile.experience_summary: '',
 ...
 const { data: existingProfile, error: existingError } = await supabase.from('candidate_profiles').select('id').eq('user_id', authData.user.id).maybeSingle();
