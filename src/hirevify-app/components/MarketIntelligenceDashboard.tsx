...
export function MarketIntelligenceDashboard({ onBack, onUpgrade }: MarketIntelligenceDashboardProps) {
 ...
 {Object.entries(salaryInsights[0].factors.skillsPremium).sort(([,a], [,b]) => b - a).map(([skill, premium]) => (
 ...
 {hiringTrends[0] && (
 <div className="space-y-3">
 {Object.entries(hiringTrends[0].industryBreakdown).sort(([,a], [,b]) => b - a).map(([industry, percentage]) => (
 ...
 {hiringTrends[0] && (
 <div className="space-y-3">
 {Object.entries(hiringTrends[0].sourceEffectiveness).sort(([,a], [,b]) => b - a).map(([source, percentage]) => (
