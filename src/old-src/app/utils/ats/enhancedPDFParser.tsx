...
export class EnhancedPDFParser {
  private commonTechnicalSkills = [
    ...
    'github actions', 'gitlab ci', 'travis ci', 'circleci', 'heroku', 'vercel', 'netlify',
  ...
  private extractPersonalInfoAdvanced(lines: string[], fullText: string): PersonalInfo {
    ...
    for (const pattern of linkedInPatterns) {
      const matches = fullText.match(pattern);
      if (matches && matches.length > 0) {
        linkedIn = matches[0].includes('linkedin.com') ? matches[0] : `linkedin.com/in/${matches[1]}`;
        break;
      }
    }
    ...
    for (const pattern of githubPatterns) {
      const matches = fullText.match(pattern);
      if (matches && matches.length > 0) {
        github = matches[0].includes('github.com') ? matches[0] : `github.com/${matches[1]}`;
        break;
      }
    }
    ...
    console.log('✅ Personal info extracted:', { fullName, email, phone, location, linkedIn, github, website, confidence });
  ...
  private containsURL(text: string): boolean {
    return /https?:\/\/|www\.|github\.com|gitlab\.com/i.test(text);
  }

  private extractURL(text: string): string | null {
    const match = text.match(/(https?:\/\/[^\s]+|www\.[^\s]+|github\.com\/[^\s]+)/i);
    return match ? match[1] : null;
  }
