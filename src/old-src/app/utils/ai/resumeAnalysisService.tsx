...
export class ResumeAnalysisService {
  ...
  private analyzePersonalInfo(personalInfo: any) {
    ...
    // Online presence
    if (personalInfo.linkedin) onlinePresence += 40;
    if (personalInfo.github) onlinePresence += 30;
    if (personalInfo.portfolio) onlinePresence += 30;

    // Professional presence
    if (personalInfo.linkedin) professionalPresence += 15;
    if (personalInfo.github) professionalPresence += 10;
    if (personalInfo.portfolio) professionalPresence += 15;

    // Recommendations
    if (!personalInfo.linkedin) {
      recommendations.push('Add LinkedIn profile to enhance professional presence');
    }
    if (!personalInfo.github && this.isTechRole(personalInfo)) {
      recommendations.push('Include GitHub profile to showcase coding projects');
    }
