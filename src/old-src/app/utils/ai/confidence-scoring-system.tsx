...
class ConfidenceScoringSystem {
  ...
  private async calculatePersonalInfoConfidence(
    ...
    const linkedIn = this.validateLinkedIn(personalInfo?.linkedIn);
  ...
  private async calculateWorkExperienceConfidence(
    ...
    const overall = this.calculateArrayAverage([
      ...companies,
      ...positions,
      ...dates,
      ...descriptions,
  ...
  private async calculateEducationConfidence(
    ...
    const overall = this.calculateArrayAverage([
      ...
      ...fields,
      ...dates,
      ...gpa.filter(g => g > 0) // Only include valid GPAs
    ]);
  ...
  private validateLinkedIn(linkedIn: string): number {
    if (!linkedIn) return 0;
    
    const linkedInRegex = /^(https?:\/\/)?(www\.)?linkedin\.com\/in\/[a-zA-Z0-9-]+\/?$/;
    return linkedInRegex.test(linkedIn) ? 95 : 30;
  }
