...
class DemoEnhancedService {
  ...
  private generateRealDataFromText(textContent: string) {
    ...
    const potentialNames = lines.slice(0, 10)
      .filter(line => {
    ...
    const linkedInMatch = textContent.match(/(?:linkedin\.com\/in\/|linkedin\.com\/pub\/)([a-zA-Z0-9\-]+)/i);
    ...
    const extractedData = {
      personalInfo: {
        ...
        website: websiteMatch?.find(url => !url.includes('linkedin') && !url.includes('github')) || '',
