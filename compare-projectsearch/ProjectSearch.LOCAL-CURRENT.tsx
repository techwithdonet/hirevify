    ...
    try {
      ...
      try {
        ...
        if (authUserId) {
          ...
          const { data: candidateProfileRows } = await supabase
            .from('candidate_profiles')
            .select('id, user_id')
            .or(candidateProfileFilters.join(','));
...
const applyToProject = async () => {
  ...
  try {
    ...
    const { data: applicationProfileRow, error: applicationProfileError } = await supabase
      .from('profiles')
    ...
    const { data: existing } = await supabase
      ...
      .eq('candidate_id', candidateOwnerProfileId)
      .maybeSingle();
...
const { error: appError } = await supabase
    ...
    await supabase
      .from('jobs')
      .update({ applications_count: (selectedProject.applicationsCount || 0) + 1 })
    ...
    if (selectedProject.recruiter_id) {
      await supabase
        .from('notifications')
        .insert({
          user_id: selectedProject.recruiter_id,
          type: 'new_application',
          title: 'New Application Received',
          message: `${candidateProfile.full_name || 'A candidate'} applied for "${selectedProject.title}"`,
          metadata: { job_id: selectedProject.id, candidate_id: candidateOwnerProfileId },
          is_read: false,
        });
    }
  ...
  useEffect(() => {
    const syncAppliedProjectsFromDatabase = async () => {
      try {
        ...
        const { data: ownerProfile } = await supabaseForAppliedState
          .from('profiles')
          .select('id, auth_user_id')
          .eq('auth_user_id', authUserId)
          .maybeSingle();
        ...
        const { data: applicationRows, error } = await supabaseForAppliedState
          .from('applications')
          .select('job_id')
        ...
        setProjects((prevProjects) =>
          prevProjects.map((project) =>
            nextAppliedIds.has(String(project.id))
    ...
    const syncAppliedStatusAfterRefresh = async () => {
      try {
        ...
        const { data: candidateProfileRow } = await supabase
          .from('candidate_profiles')
          .select('id, user_id')
          .or(`user_id.eq.${authUserId}${profileRow?.id ? `,user_id.eq.${profileRow.id}` : ''}`)
          .maybeSingle();
        ...
        setProjects((currentProjects) =>
          currentProjects.map((project) =>
            appliedIds.has(String(project.id))
              ? { ...project, status: 'applied' as const }
              : project
          )
        );
                    ...
                    <Card key={project.id} className="p-6 hover:shadow-md transition-shadow cursor-pointer">
                      <div className="space-y-4">
                        ...
                        <div className="flex items-center justify-between pt-2">
                          ...
                          ) : selectedProject?.status && ['in-progress', 'completed'].includes(selectedProject?.status) && !project.hasVideoSubmission ? (
                            <Button 
                              size="sm"
